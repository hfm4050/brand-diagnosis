...
출력: 1920×1080 / 30fps / H.264 + AAC. 이 컨테이너(4코어)에서 약 1분.

프레임 캡처는 코어 수만큼 크로미움을 띄워 병렬로 찍는다. 스크린샷은 브라우저
프로세스 안에서 직렬화되므로 페이지만 여러 개 띄우면 빨라지지 않는다.
`CAPTURE_WORKERS` 로 개수를 조절한다 (기본 = CPU 코어 수).
